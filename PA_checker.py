...
def run_tests(cmd, test_dir, method_python):
    ...
    wa_count = 0
    log_name = (
        test_dir + f"/auto_log_py.txt"
        if method_python
        else test_dir + f"/auto_log_cpp.txt"
    )
    fres = open(log_name, "w")
    for cond in ["true", "false"]:
        # filter out those end with '_out.txt'
        files = [f for f in os.listdir("./tests/" + cond) if not f.endswith("_out.txt")]
        for file in files:
            path = file.split(".")
            ext = path[-1]
            if ext != "txt":
            ...
            (res, desc) = cs.run(
                cmd + " ./tests/" + cond + "/" + name + ".txt", time_limit
            )
            if res == 1:
                tle_count += 1
...
def run_py_tests(test_dir):
    ...
    return run_tests(
        python_executable + test_dir + "/kill_Down_with_Trojans.py", test_dir, True
    )
...
if __name__ == "__main__":
    ...
    your_repo_dir = (
        "/Users/rasmushenriksen/Desktop/UCLA/Quarters/CS180-Programming-Assignment"
    )
